...
public class DefaultAdopterService implements AdopterService {
	...
	public void init() throws IOException {
		...
		Gson gson = new Gson();
		try (InputStream s = cl.getResourceAsStream("adopters.json");
	...
	public List<AdoptedProject> getAdoptedProjects(List<Project> projects) {
		return projects.stream().map(this::getAdoptedProject).filter(a -> !a.getAdopters().isEmpty()).collect(Collectors.toList());
	}
	...
	private AdoptedProject getAdoptedProject(Project p) {
		...
		ap.setUrl(p.getUrl());
		ap.setAdopters(
				getAdopters().stream().filter(a -> a.getProjects().contains(p.getProjectId())).collect(Collectors.toList()));
		return ap;
	}
